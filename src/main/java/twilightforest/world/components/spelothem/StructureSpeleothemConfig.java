...
public record StructureSpeleothemConfig(
	...
	private static Function<RandomSource, Stalactite> compileStalactites(Supplier<SpeleothemVarietyConfig> varietyConfigSupplier) {
		...
		ArrayList<Weighted<Stalactite>> unbakedRandomList = stalactites.stream().map(s -> Weighted.wrap(s, Mth.ceil(s.weight() * stoneCounterweight))).collect(Collectors.toCollection(ArrayList::new));
		// Add oreStalactites to the unbaked list
		oreStalactites.stream().map(s -> Weighted.wrap(s, Mth.ceil(s.weight() * oreCounterweight))).forEachOrdered(unbakedRandomList::add);
	...
	private static Function<RandomSource, Stalactite> compileSpeleothemsSimple(List<Stalactite> stalactites) {
		return compileSpeleothems(stalactites.stream().map(stalactite -> Weighted.wrap(stalactite, stalactite.weight())).toList());
	}
	...
	private static Function<RandomSource, Stalactite> compileSpeleothems(List<Weighted<Stalactite>> unbakedRandomList) {
		...
		return random -> randomList.getRandom(random).map(Weighted::data).orElse(BlockSpikeFeature.STONE_STALACTITE);
	}
