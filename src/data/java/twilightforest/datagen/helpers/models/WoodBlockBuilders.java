...
public abstract class WoodBlockBuilders extends BlockModelGenerators {
	...
	public WoodBlockBuilders(Consumer<BlockStateGenerator> stateOutput, ItemModelOutput itemOutput, BiConsumer<Identifier, ModelInstance> modelOutput) {
	...
	public void generateHangingSign(Block ceiling, Block wall, Block particle) {
		Identifier resourcelocation = ModelTemplates.PARTICLE_ONLY.create(ceiling, new TextureMapping().put(TextureSlot.PARTICLE, TextureMapping.getBlockTexture(particle)), this.modelOutput);
	...
	public void generateChiseledBookshelf(Block shelf) {
		Identifier resourcelocation = ModelLocationUtils.getModelLocation(shelf);
	...
	public void addBookSlotModel(Block shelf, MultiPartGenerator generator, Condition.TerminalCondition condition, VariantProperties.Rotation rotation, BooleanProperty property, ModelTemplate template, boolean occupied) {
		...
		Identifier resourcelocation = CHISELED_BOOKSHELF_SLOT_MODEL_CACHE.computeIfAbsent(cache, key -> template.createWithSuffix(shelf, suffix, texturemapping, this.modelOutput));
		generator.with(
